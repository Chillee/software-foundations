...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  ...
  match s with
    | h::ss => match beq_nat v h with
                 | true => ss
                 | false => h :: remove_one v ss
               end
    | nil => nil
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
    | h::ss => match beq_nat v h with
                 | true => remove_all v ss
                 | false => h :: remove_all v ss
               end
    | nil => nil
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
    | h::ss => andb (member h s2) (subset ss (remove_one h s2))
    | nil => true
  end.
...
Proof. intros n l. simpl. rewrite <- beq_nat_refl. reflexivity. Qed.
 (** [] *)
...
Theorem rev_length_firsffttry : forall l : natlist,
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
    | nil, nil => true
    | a :: l1', b :: l2' => andb (beq_nat a b) (beq_natlist l1' l2')
    | _, _ => false
  end.
...
Proof.
  ...
  reflexivity.
  Case "l = n :: l'".
  simpl. rewrite <- beq_nat_refl. simpl. rewrite -> IHl'. reflexivity.
Qed.
(** [] *)
...
Theorem foo : forall c : nat, forall l : bag,
                cons c l = [c] ++ l.
  intros c l. simpl. reflexivity.
Qed.

Theorem cons_snoc_app : forall c : nat, forall n m : bag,
                          n ++ cons c m = snoc n c ++ m.
Proof.
  intros c n m. rewrite -> snoc_append. rewrite -> app_assoc. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros a b n. induction a as [| m a'].
  simpl. reflexivity.
  simpl.
