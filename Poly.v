...
Proof.
  intros X l1 l2 v. induction l1 as [| l1' ].
  reflexivity.
...
Proof.
  intros X Y Z f p. destruct p. simpl. reflexivity.
Qed.
(** [] *)
...
Lemma map_snoc : forall (X Y : Type) (f : X -> Y) (l : list X) (l' : X),
                   map f (snoc l l') = snoc (map f l) (f l').
Proof.
  intros X Y f l l'. induction l as [| h t].
  reflexivity.
...
Proof.
  intros X l. induction l as [| l' ].
  reflexivity.
  simpl. unfold fold_length. simpl. rewrite <- IHl. unfold fold_length. reflexivity.
Qed.
(** [] *)
...
Theorem fold_map_correct : forall (X Y : Type) (f : X -> Y) (l : list X) (m : list Y),
...
Definition exp (n m : nat) : nat :=
  fun (X : Type) (f : X -> X) (x : X) => m (X -> X) (n X) f x. (* works :) *)
  (* fun (X : Type) (f : X -> X) (x : X) => (fold mult (n (list nat) (cons m) []) one) X f x. (* gives universe inconsistency *) *)
  (* fun (X : Type) (f : X -> X) (x : X) => (n nat (mult m) one) X f x. (* gives universe inconsistency *) *)
